...
/// <summary>
/// Optional production initialization seam for an authoritative semantic provider. Runtime
/// doubles need not implement it; the shipped RuntimeBridge singleton does, and the controller
/// binds the factory before the first live pre-entry capture.
/// </summary>
internal interface INetherTypedSemanticProviderRegistration
{
    void RegisterTypedSemanticProviderFactory(
...
/// </summary>
internal sealed class NetherRuntimeBridge : NetherOwnedPopupStageBridgeAdapter, INetherRuntimeBridge, INetherTypedSemanticProviderRegistration, INetherCheckpointPopupWaitDriver, INetherOwnedPopupNativeStagePort
{
    ...
    private NetherRuntimeNativeEventPopupCaptureFactory? _nativeEventPopupCaptureFactory;
    private NetherRuntimeManagedShopPopupCaptureFactory? _managedShopPopupCaptureFactory;
    ...
    private long _recoveredCodeObservedSequence;
    private object? _battleResultViewController;
    private long _battleResultCodeGeneration;
    private long _battleResultCodeObservedSequence;
    ...
    internal NetherRuntimeBridge(NetherRuntimeTypedSemanticProviderFactory? typedSemanticProviderFactory = null)
    {
        _typedSemanticProviderFactory = typedSemanticProviderFactory;
        _checkpointPopupWait = new NetherCheckpointPopupWaitCoordinator(this);
    }

    /// <summary>
    /// Binds an optional authoritative semantic provider factory. The factory is invoked with the
    /// current immutable snapshot and its returned scope must carry the same fingerprint before
    /// any typed reward or battle tier can reach the visible mapper. Null remains the native
    /// default because raw type/rarity fields do not prove those semantics.
    /// </summary>
    public void RegisterTypedSemanticProviderFactory(NetherRuntimeTypedSemanticProviderFactory? factory)
    {
        lock (_gate)
    ...
    /// <summary>
    /// Optional raw Event popup capture adapter for a host that already owns the native field
    /// copy. The adapter carries raw IDs/types/rarities only; semantic Battle/Reward tiers still
    /// require the separate snapshot-scoped typed provider. Null preserves the default
    /// Engine/MasterDataStore capture path.
    /// </summary>
    public void RegisterNativeEventPopupCaptureFactory(
        NetherRuntimeNativeEventPopupCaptureFactory? factory
    )
    {
        lock (_gate)
            _nativeEventPopupCaptureFactory = factory;
    }

    /// <summary>
    /// Optional managed Shop DTO adapter. The DTO must carry the current snapshot fingerprint;
    /// this path is equivalent to the native Shop mapper but never loads an IL2CPP/GameAssembly
    /// type. Raw content type/ID/amount remain correlation data and cannot prove a key.
    /// </summary>
    public void RegisterManagedShopPopupCaptureFactory(
        NetherRuntimeManagedShopPopupCaptureFactory? factory
    )
    {
        lock (_gate)
            _managedShopPopupCaptureFactory = factory;
    }

    internal NetherRuntimeInteractivePreEntryCaptureResult CaptureInteractivePreEntryFloor(
        NetherSnapshot snapshot,
        NetherAutoClimbSettings settings,
        object floorModel,
        ...
        IEnumerable? itemRows,
        IEnumerable? battleRows,
        long floorNodeId,
        bool canCloseShop
    )
    {
        if (snapshot == null || settings == null || floorModel == null)
            return new NetherRuntimeInteractivePreEntryCaptureResult
            {
                IsCaptured = false,
                Detail = "invalid-runtime-bridge-interactive-capture-input",
            };

        return InteractivePreEntryInputCapture.Capture(
            new NetherRuntimeInteractivePreEntryCaptureRequest(
                FloorModel: floorModel,
                MapFloorRows: mapFloorRows,
                EventRows: eventRows,
            ...
            )
            {
                ...
                FloorNodeId = floorNodeId,
                ItemRows = itemRows,
                BattleRows = battleRows,
                TypedSemanticProvider = ResolveTypedSemanticProvider(snapshot),
            }
        );
    }

    /// <summary>
    /// Managed adapter seam for hosts that have already copied the native pre-entry fields into
    /// reflection-shaped DTOs. It uses the same pure capture/evaluation path as the native bridge,
    /// then stores the exact snapshot-scoped result consumed by popup binding. No IL2CPP type is
    /// loaded by this seam; the default production path remains the native cache reader above.
    /// </summary>
    internal NetherRuntimeInteractivePreEntryInputsResult CaptureManagedInteractivePreEntryFloor(
        NetherSnapshot snapshot,
        NetherAutoClimbSettings settings,
        object floorModel,
        IEnumerable? mapFloorRows,
    ...
    {
        if (snapshot == null || settings == null || floorModel == null || floorNodeId <= 0)
        {
            return NetherRuntimeInteractivePreEntryInputsResult.Failure(
                "invalid-managed-interactive-preentry-capture-input"
            );
        }

        ObserveAuthoritativeRouteSnapshot(snapshot.Fingerprint);
        NetherRuntimeInteractivePreEntryCaptureResult captured = CaptureInteractivePreEntryFloor(
            snapshot,
            settings,
            floorModel,
            mapFloorRows,
        ...
        if (!captured.IsCaptured || captured.Input == null)
        {
            return NetherRuntimeInteractivePreEntryInputsResult.Failure(
                "managed-interactive-preentry-capture:" + captured.Detail
            );
        }

        NetherRuntimeInteractivePreEntryInputsResult result =
            NetherRuntimeInteractivePreEntryInputsResult.Success(
                new Dictionary<long, NetherRuntimeInteractivePreEntryCaptureResult>
                {
                    [floorNodeId] = captured,
                },
                snapshot.Fingerprint,
                captured.Input.TypedSemanticProvider
            );
        lock (_gate)
        {
            _latestInteractivePreEntryInputs = result;
            _latestStrategySettings = settings;
        }
        return result;
    }

    private NetherStrategyTypedSemanticProviderEvidence? ResolveTypedSemanticProvider(
        NetherSnapshot snapshot
    )
    {
        NetherRuntimeTypedSemanticProviderFactory? factory;
        lock (_gate)
            factory = _typedSemanticProviderFactory;
        if (factory == null)
            return null;

        try
        {
            NetherRuntimeTypedSemanticProviderScope? scope = factory(snapshot);
            if (scope == null
                || scope.Evidence == null
                || scope.SnapshotFingerprint != snapshot.Fingerprint)
            {
                return null;
            }
    ...
    public bool HasRecoveredCodeOffer
    {
        get
        {
            lock (_gate)
            {
                return !_recoveredCheckpointHandoffPrepared
    ...
    )
    {
        ...
        NetherStrategyTypedSemanticProviderEvidence? typedSemanticProvider =
            ResolveTypedSemanticProvider(snapshot);
    ...
    public NetherRuntimeCodeCandidatesResult TryGetCodeCandidates()
    {
        try
        {
            ...
            PopupRegistration? coverageRegistration;
            lock (_gate)
            {
                coverageRegistration = _codeSelectPopup is PopupRegistration current
                    && IsCurrentFloorOwnedPopup(current)
                        ? current
                        : null;
            }
            ...
            // Ability scope/target/situations are asset semantics, not category or power fields.
            // Retain a bounded audit for those exact runtime assets without guessing coverage.
                ...
                foreach (object rawCode in Enumerate(dataStore.GetPossessionNetherCodeDataEnumerable()))
                {
                        ...
                        && masterById.TryGetValue(code.MNetherCodeId, out MNetherCodes? master))
                    {
                        NetherCodeState current = NetherCodeRuntimeSemanticMapper.MapState(
                            ...
                            master.category,
                            master.effect_type,
                            master.effect_parameter_1,
                            master.effect_parameter_2,
                            master.effect_parameter_3,
                            master.rarity,
                            master.power,
                            code.Amount
                        );
                        if (!current.IsKnown
                            || !current.EffectSemanticsKnown
                            || current.MasterEffectType is NetherCodeMasterEffectType.NetherAbility
            ...
            var candidatePartyCoverage = new Dictionary<long, int>();
            var currentPartyCoverage = new Dictionary<long, int>();
            if (coverageRegistration is PopupRegistration coverageOwner)
            {
                bool coverageComplete = true;
                foreach (long codeId in candidates.Select(candidate => candidate.CodeId).Distinct())
                {
                    if (!TryGetNativeCodeDisplayCoverage(coverageOwner, codeId, out int coverage))
                    {
                        coverageComplete = false;
                        break;
                    }
                    candidatePartyCoverage[codeId] = coverage;
                }

                if (coverageComplete)
                {
                    foreach (long codeId in Enumerate(dataStore.GetPossessionNetherCodeDataEnumerable())
                        .OfType<NetherCodeData>()
                        .Where(code => code != null && code.MNetherCodeId > 0 && code.Amount > 0)
                        .Select(code => code.MNetherCodeId)
                        .Distinct())
                    {
                        if (!TryGetNativeCodeDisplayCoverage(coverageOwner, codeId, out int coverage))
                        {
                        ...
                        currentPartyCoverage[codeId] = coverage;
                    }
                }

                lock (_gate)
                    coverageComplete = coverageComplete && IsSameCurrentCodeOfferRegistration(coverageOwner);
                if (!coverageComplete)
                {
                    candidatePartyCoverage.Clear();
                    currentPartyCoverage.Clear();
                }
            }

            if (candidatePartyCoverage.Count > 0)
            {
                candidates = candidates.Select(candidate =>
                    candidatePartyCoverage.TryGetValue(candidate.CodeId, out int coverage)
                        ? candidate with
                        {
            ...
            return new NetherRuntimeCodeCandidatesResult(
                candidates,
                candidates.All(candidate => candidate.IsKnown),
                string.Empty
            )
            {
                CurrentPartyCoverage = currentPartyCoverage,
            };
        }
        catch (Exception ex)
        {
        ...
        lock (_gate)
        {
                ...
                })
            {
                if (candidate is { IsLive: true }
                    && candidate.Value.RuntimeGeneration == _runtimeGeneration
    ...
    private NetherRuntimePopupResult TryMapPopupRegistration(PopupRegistration registration)
    {
        ...
        NetherStrategyTypedSemanticProviderEvidence? typedSemanticProvider;
        NetherRuntimeNativeEventPopupCaptureFactory? nativeEventPopupCaptureFactory;
        NetherRuntimeManagedShopPopupCaptureFactory? managedShopPopupCaptureFactory;
        NetherSnapshotFingerprint? authoritativeSnapshotFingerprint;
        lock (_gate)
        {
            if (!registration.IsLive
                || registration.RuntimeGeneration <= 0
                || registration.RuntimeGeneration != _runtimeGeneration)
            {
                return NetherRuntimePopupResult.Failure(
                    "stale-popup-runtime-generation:registered="
            ...
            authoritativeSnapshotFingerprint = _authoritativeRouteSnapshotFingerprint;
            typedSemanticProvider = _latestInteractivePreEntryInputs?.TypedSemanticProvider;
            nativeEventPopupCaptureFactory = _nativeEventPopupCaptureFactory;
            managedShopPopupCaptureFactory = _managedShopPopupCaptureFactory;
            if (authoritativeSnapshotFingerprint is not NetherSnapshotFingerprint currentFingerprint
                || _latestInteractivePreEntryInputs?.SnapshotFingerprint != currentFingerprint)
            {
        ...
        try
        {
            NetherRuntimePopupResult mapped = controllerType switch
            {
                ...
                EventPopupControllerTypeName => TryMapEventPopup(
                    registration,
                    NetherRuntimePopupKind.Event,
                    (int)NetherFloorNodeType.Event,
                    nativeEventPopupCaptureFactory,
                    typedSemanticProvider,
                    authoritativeSnapshotFingerprint
                ),
                RecoverPopupControllerTypeName => TryMapEventPopup(
                    registration,
                    NetherRuntimePopupKind.Recovery,
                    (int)NetherFloorNodeType.Recovery,
                    nativeEventPopupCaptureFactory,
                    typedSemanticProvider,
                    authoritativeSnapshotFingerprint
                ),
                TreasurePopupControllerTypeName => TryMapEventPopup(
                    registration,
                    NetherRuntimePopupKind.Treasure,
                    (int)NetherFloorNodeType.Treasure,
                    nativeEventPopupCaptureFactory,
                    typedSemanticProvider,
                    authoritativeSnapshotFingerprint
                ),
                ShopPopupControllerTypeName => TryMapShopPopup(
                    registration,
                    typedSemanticProvider,
                    managedShopPopupCaptureFactory,
                    authoritativeSnapshotFingerprint
                ),
                _ => NetherRuntimePopupResult.Failure("unsupported-native-popup-controller:" + controllerType),
            };
            if (mapped.Popup == null)
                return mapped;

            NetherRuntimePopupContext context = mapped.Popup with
            {
            ...
            if (authoritativeSnapshotFingerprint is not NetherSnapshotFingerprint currentInteractiveFingerprint
                || interactiveInputs?.SnapshotFingerprint != currentInteractiveFingerprint)
            {
                interactiveInputs = null;
            }
    ...
    /// counted member satisfies the ability's runtime Target/Situations trigger.  The capture is
    /// optional and is accepted only while the exact popup owner remains unchanged before and
    /// after every native call.
    /// </summary>
    private bool TryGetNativeCodeDisplayCoverage(
        PopupRegistration expected,
        long codeId,
        out int coverage
    )
    {
        coverage = 0;
        Project.Nether.NetherPartyModel? partyModel;
        lock (_gate)
        {
            if (!IsSameCurrentCodeOfferRegistration(expected)
                || !TryReadMember(expected.Controller, "_partyModel", out object? rawPartyModel)
                || rawPartyModel is not Project.Nether.NetherPartyModel mappedPartyModel)
            {
                return false;
            }
            partyModel = mappedPartyModel;
        }

        try
        {
            var characters = partyModel.GetValidCharacterModels();
            Project.Nether.NetherCodeModel? codeModel =
                Project.Nether.NetherCodeModel.CreateModel(codeId);
            if (characters == null || codeModel == null)
                return false;

            int mappedCoverage = Project.Nether.NetherAbyssCodePowerCalculator.GetBuffTargetCount(
                codeModel,
                characters
            );
            if (mappedCoverage < 0)
    ...
    /// <summary>Requires the bridge lock; compares the complete immutable popup owner identity.</summary>
    private bool IsSameCurrentCodeOfferRegistration(PopupRegistration expected) =>
        _codeSelectPopup is PopupRegistration current
        && current.IsLive
        && ReferenceEquals(current.Controller, expected.Controller)
        && ReferenceEquals(current.Popup, expected.Popup)
        && current.RuntimeGeneration == expected.RuntimeGeneration
        && current.OwnerAction == expected.OwnerAction
    ...
    private static NetherRuntimePopupResult TryMapCodeSelectPopup(PopupRegistration registration)
    {
        ...
        NetherRuntimePopupContext context = new() { Kind = NetherRuntimePopupKind.CodeOffer };
        if (readiness.IsReady)
            return NetherRuntimePopupResult.Success(context);
        return readiness.IsPending
            ? NetherRuntimePopupResult.Pending(context, readiness.Detail)
    ...
    public NetherRuntimePopupResult TryGetBattleResultCodePopup()
    {
        ...
        lock (_gate)
        {
            ...
            observedSequence = _battleResultCodeObservedSequence;
        }
        PopupRegistration candidate = registration.GetValueOrDefault();
        NetherOwnedCodePopupRegistrationDecision readiness =
        ...
        if (readiness.IsAwaiting)
        {
            return NetherRuntimePopupResult.Pending(
        ...
        if (!readiness.IsReady)
            return NetherRuntimePopupResult.Failure(readiness.Detail);
    ...
    public NetherRuntimePopupResult TryGetRecoveredCodePopup()
    {
        ...
        lock (_gate)
        {
            ...
            runtimeGeneration = _runtimeGeneration;
            observedSequence = _recoveredCodeObservedSequence;
        }
        PopupRegistration candidate = registration.GetValueOrDefault();
        NetherOwnedCodePopupRegistrationDecision readiness =
            NetherOwnedCodePopupRegistrationReadiness.Evaluate(
    ...
    public NetherBattleResultCodeNativeStep PollRecoveredCodeNative()
    {
        ...
        if (_recoveredCodeDiagnosticGate.ShouldEmit(
                "native",
    ...
    public NetherNativeActionResult PollRecoveredCodeParent()
    {
        ...
        if (_recoveredCodeDiagnosticGate.ShouldEmit(
                "parent",
                generation + "|" + result.Kind + "|" + result.Detail
            ))
        {
            NetherAutoClimbController.LogDiagnostic(
    ...
    public NetherRecoveredCheckpointObservation ObserveRecoveredCheckpoint()
    {
        ...
        lock (_gate)
        {
            ...
            {
                return NetherRecoveredCheckpointObservation.NotObserved(
            ...
            {
                return NetherRecoveredCheckpointObservation.NotObserved(
                    "no-existing-start-status-checkpoint-parent"
                );
            }
            ...
            if (registration is not PopupRegistration candidate
                ...
                || candidate.RuntimeGeneration != _runtimeGeneration)
            {
                return NetherRecoveredCheckpointObservation.Waiting(
                    "existing-checkpoint-continue-popup-not-yet-registered"
                );
            }
            if (candidate.OwnerAction != NetherActionKind.None
                || candidate.OwnerGeneration != 0)
            {
                return NetherRecoveredCheckpointObservation.BindingUnavailable(
                    "existing-checkpoint-continue-popup-already-owned:"
                        + candidate.OwnerAction + ":" + candidate.OwnerGeneration
                );
            }
            if (!_startStatusParentCapture.TryAttachPopup(_floorSelectionController)
                || !_startStatusParentCapture.TryGetParentObservation(
                    _floorSelectionController,
                    out NetherStartStatusParentObservation parent
                ))
            {
                return NetherRecoveredCheckpointObservation.BindingUnavailable(
                    "existing-checkpoint-parent-attachment-unavailable"
                );
            }
            if (parent.State != NetherStartStatusParentState.Pending)
            {
                return NetherRecoveredCheckpointObservation.BindingUnavailable(
                    "existing-checkpoint-parent-not-pending:" + parent.State + ":" + parent.Detail
                );
            }
        }

        NetherRuntimeSnapshotResult snapshot = TryCaptureSnapshot();
        if (!snapshot.IsSuccess || snapshot.Snapshot == null)
        {
            return NetherRecoveredCheckpointObservation.BindingUnavailable(
                "existing-checkpoint-snapshot:" + snapshot.Detail
            );
        }
        if (snapshot.Snapshot.Status != NetherSessionStatus.Sleep)
        {
            return NetherRecoveredCheckpointObservation.BindingUnavailable(
                "existing-checkpoint-wrong-status:" + snapshot.Snapshot.Status
            );
        }
        return NetherRecoveredCheckpointObservation.Ready(
            snapshot.Snapshot,
            "existing-start-status-continue-popup:sequence=" + registration.Value.Sequence
        );
    }
    ...
    public NetherNativeActionResult PrepareRecoveredCheckpointHandoff()
    {
        ...
        {
            if (_recoveredCheckpointHandoffPrepared)
                return NetherNativeActionResult.Rejected("recovered-checkpoint-handoff-already-prepared");
            if (_floorSelectionController == null
                || !_startStatusParentCapture.TryGetParentObservation(
                    _floorSelectionController,
                    out NetherStartStatusParentObservation parent
                )
                || parent.State != NetherStartStatusParentState.Pending
                || _continuePopup is not PopupRegistration registration
                || !registration.IsLive
                || registration.RuntimeGeneration <= 0
                || registration.RuntimeGeneration != _runtimeGeneration
                || registration.OwnerAction != NetherActionKind.None
                || registration.OwnerGeneration != 0)
            {
                return NetherNativeActionResult.BindingUnavailable(
                    "existing-checkpoint-handoff-evidence-changed"
                );
            }

            _recoveredCheckpointHandoffPrepared = true;
            return NetherNativeActionResult.Completed(
                "existing-checkpoint-handoff-prepared:sequence=" + registration.Sequence
            );
        }
    }
    ...
    private NetherBattleResultCodeNativeStep LogBattleResultCodeNativeStep(
        NetherBattleResultCodeNativeStep step
    )
    {
        if (_battleResultCodeDiagnosticGate.ShouldEmit(
                "native",
                step.Kind + "|" + step.Detail
            ))
        {
    ...
    public NetherBattleResultContinuationStep PollBattleResultContinuation(bool allowInvoke)
    {
        lock (_gate)
        {
            ...
            NetherRuntimePopupResult popup = snapshot.Status == NetherSessionStatus.Wait
                ? TryGetActivePopup()
                : NetherRuntimePopupResult.Failure("popup-not-required");
            if (snapshot.Status == NetherSessionStatus.Wait && popup.IsPending)
            {
                const string waitBoundary = "battle-result-rebound-modal";
                string waitDetail = popup.Detail;
                NetherNativeActionResult wait = _battleResultPopupReadiness.Await(
                    NetherPopupReadinessIdentity.From(popup.Popup!),
                    waitBoundary
                );
                if (wait.Kind == NetherNativeActionResultKind.Started)
                {
            ...
            if (snapshot.Status == NetherSessionStatus.Wait && !popup.IsSuccess)
            {
            ...
            _battleResultPopupReadiness.ObserveReady();
    ...
    public void ClearRegistrations()
    {
        lock (_gate)
        {
            ...
            _startStatusCodeGeneration = 0;
            _recoveredCodeObservedSequence = 0;
            _battleResultViewController = null;
            _battleResultCodeGeneration = 0;
            _battleResultCodeObservedSequence = 0;
            _battleResultCharactersRequired = false;
            _floorParentAction = null;
            _floorParentGeneration = 0;
            _floorEventSequenceTaskFlow.Reset();
    ...
    private void RegisterFloorSelectionCore(object controller, string source)
    {
        ...
        lock (_gate)
        {
            ...
            if (replaced)
            {
                ...
                ClearRecoveredCodeOfferCore();
                ClearRuntimePopupRegistrationsForControllerReplacementCore();
    ...
    /// <summary>
    /// A popup/controller registration is valid only for the floor runtime generation that
    /// created it.  Preserve independent Continue/Result parent-task evidence, but discard every
    /// visual callback and child task before advancing to a replacement FloorSelection owner.
    /// </summary>
        ...
        lock (_gate)
        {
            ...
            else if (isCodeReplacementSupportPopup)
            {
                ...
                if (expectedStage
                    ...
                    && replacementList.RuntimeGeneration == _runtimeGeneration
            ...
            switch (typeName)
            {
                ...
                case ShopConfirmPopupControllerTypeName:
                    ...
                    _shopConfirmParentSequence = _shopPopup is PopupRegistration shop
                        && shop.IsLive
                        && shop.RuntimeGeneration == _runtimeGeneration
                ...
                case CodeSelectPopupControllerTypeName:
                    ...
                    if (ownerAction == NetherActionKind.BattleSettlement)
                        _battleResultCodeObservedSequence = sequence;
                    else if (ownerAction == NetherActionKind.RecoveredCodeOffer)
                        _recoveredCodeObservedSequence = sequence;
                    break;
                ...
                case CodeReplacementConfirmPopupControllerTypeName:
                    ...
                    codeReplacementConfirmBound =
                        ...
                        && currentReplacementList.RuntimeGeneration == _runtimeGeneration
                ...
                case CodeReplacementCompletePopupControllerTypeName:
                    ...
                    if (ownerAction != NetherActionKind.None
                        ...
                        && completeReplacementList.RuntimeGeneration == _runtimeGeneration
        ...
        lock (_gate)
        {
            ...
            if (_returnScrollController is CheckpointControllerRegistration scroll
                && (ReferenceEquals(scroll.Controller, popup)
                    || ReferenceEquals(scroll.ParentPopup, popup)))
            {
                _returnScrollController = null;
            }
        }
    ...
    private bool IsCurrentCheckpointRegistration(
        ...
        && returnPopup.IsLive
        && returnPopup.RuntimeGeneration == _runtimeGeneration
        && ReferenceEquals(registration.ParentPopup, returnPopup.Popup)
    ...
    private void ClearFloorParentCore()
    {
        ...
        ClearFloorPopup(ref _codeReplacementConfirmPopup, generation);
    ...
    private void ClearBattleResultCodeOwnerCore()
    {
        ...
        ClearBattleResultPopup(ref _codeReplacementConfirmPopup, generation);
        ClearBattleResultPopup(ref _codeReplacementCompletePopup, generation);
        ClearBattleResultPopup(ref _codeTransformConfirmPopup, generation);
        ClearBattleResultPopup(ref _codeTransformCompletePopup, generation);
        _battleResultViewController = null;
        _battleResultCodeGeneration = 0;
        _battleResultCodeObservedSequence = 0;
        _battleResultPopupReadiness.Clear();
    ...
    private void ClearRecoveredCodeOfferCore()
    {
        ...
        ClearRecoveredCodePopup(ref _codeReplacementConfirmPopup, generation);
        ClearRecoveredCodePopup(ref _codeReplacementCompletePopup, generation);
        ClearRecoveredCodePopup(ref _codeTransformConfirmPopup, generation);
        ClearRecoveredCodePopup(ref _codeTransformCompletePopup, generation);
    ...
    private void RegisterReturnScrollCore(object controller)
    {
        ...
        lock (_gate)
        {
            // InitializePopup in the shipped client can bypass this wrapper hook. Once the
            // main-thread checkpoint poll has derived the exact popup-owned scroll, a later
            // OnThumbnailClicked postfix must not replace it with a newer uncorrelated entry.
            if (_returnScrollController is CheckpointControllerRegistration existing
                && existing.IsLive
                && existing.RuntimeGeneration == _runtimeGeneration
                && existing.ParentPopup != null
    ...
    private bool TryBindReturnScrollFromCurrentPopupCore(string source)
    {
        if (_returnPopup is not PopupRegistration registration)
            return false;
        return TryBindReturnScrollFromPopupCore(registration, source);
    }

    private bool TryBindReturnScrollFromPopupCore(
        PopupRegistration registration,
        string source
    )
    {
        if (!registration.IsLive
            ...
            || registration.Sequence <= _checkpointMinimumSequence)
        {
            LogReturnScrollBinding(
                source,
                registration,
                new(null, 0, 0, "stale-return-popup-owner")
            );
            return false;
        }
        ...
        if (_returnScrollController is CheckpointControllerRegistration existing
            && existing.IsLive
            && existing.RuntimeGeneration == registration.RuntimeGeneration
            && ReferenceEquals(existing.ParentPopup, registration.Popup)
            && existing.OwnerAction == registration.OwnerAction
            && existing.OwnerGeneration == registration.OwnerGeneration
            && existing.Sequence == registration.Sequence)
        {
            return true;
        }

        NetherCheckpointReturnScrollResolution resolution =
            NetherCheckpointReturnScrollResolver.Resolve(
                registration.Popup,
        ...
        if (resolution.IsReady)
        {
            _returnScrollController = new CheckpointControllerRegistration(
                ...
                registration.RuntimeGeneration,
                IsLive: true,
                ParentPopup: registration.Popup
            );
        }
        LogReturnScrollBinding(source, registration, resolution);
        return resolution.IsReady;
    }

    private void LogReturnScrollBinding(
        string source,
        PopupRegistration registration,
        NetherCheckpointReturnScrollResolution resolution
    )
    {
        string signature = source + "|" + registration.Sequence + "|" + resolution.Detail;
        if (!_checkpointDiagnosticGate.ShouldEmit("return-scroll-binding", signature))
            return;
        NetherAutoClimbController.LogDiagnostic(
            "checkpoint-return-scroll",
            new("source", source),
            new("outcome", resolution.IsReady ? "bound" : "waiting"),
            new("popupSequence", registration.Sequence.ToString()),
            new("owner", registration.OwnerAction.ToString()),
            new("ownerGeneration", registration.OwnerGeneration.ToString()),
            new("controllerType", resolution.Controller?.GetType().FullName ?? "null"),
            new("contentCount", resolution.ContentCount.ToString()),
            new("selectionLimit", resolution.SelectionLimit.ToString()),
            new("detail", resolution.Detail)
        );
    }
    ...
    private void ObserveStartStatusStateMachineExitCore(object stateMachine)
    {
        ...
        bool terminalFaultObserved = string.Equals(taskStatus, "Faulted", StringComparison.Ordinal)
            || string.Equals(taskStatus, "Canceled", StringComparison.Ordinal)
            || !string.IsNullOrEmpty(builderException);
        ...
        if (terminalFaultObserved)
        {
            NetherAutoClimbController.LogDiagnostic(
                "runtime-fault",
                new("action", "start-status-parent-terminal"),
                new("accepted", accepted.ToString()),
                new("ownerGeneration", generation.ToString()),
                new("runnerIdentity", runnerIdentity.ToString(CultureInfo.InvariantCulture)),
                new("state", state.ToString(CultureInfo.InvariantCulture)),
                new("taskStatus", taskStatus),
                new("builderException", string.IsNullOrEmpty(builderException) ? "-" : builderException),
                new("faultException", string.IsNullOrEmpty(faultDiagnostic.ExceptionSummary) ? "-" : faultDiagnostic.ExceptionSummary),
                new("faultSource", faultDiagnostic.Source),
                new("faultProbe", faultDiagnostic.Probe),
                new("ready", ready.ToString()),
                new("adoptedExistingPopup", adoptedExistingPopup.ToString())
            );
        }
    ...
    private bool TryAdoptRecoveredCodePopupCore()
    {
        ...
        _recoveredCodeObservedSequence = ownership.Sequence;
        return true;
    }
    ...
    private NetherNativeActionResult ConfirmCodeReplacement()
    {
        ...
        if (!confirmation.Value.IsLive
            || confirmation.Value.RuntimeGeneration != _runtimeGeneration
    ...
    private NetherNativeActionResult DismissCodeReplacementCompleteIfNeeded()
    {
        ...
        if (!complete.Value.IsLive
            || complete.Value.RuntimeGeneration != _runtimeGeneration
    ...
    )
    {
        ...
        if (registration is not PopupRegistration list
            || !list.IsLive
            || list.RuntimeGeneration != _runtimeGeneration
    ...
    private NetherNativeActionResult StartCheckpointNativeFlow(NetherPlannedAction action)
    {
        ...
        lock (_gate)
        {
            ...
            if (_recoveredCheckpointHandoffPrepared
                && _continuePopup is PopupRegistration existingContinue)
            {
                if (!existingContinue.IsLive
                    || existingContinue.RuntimeGeneration <= 0
                    || existingContinue.RuntimeGeneration != _runtimeGeneration
                    || existingContinue.OwnerAction != NetherActionKind.None
                    || existingContinue.OwnerGeneration != 0
                    || _floorSelectionController == null
                    || !_startStatusParentCapture.TryGetParentObservation(
                        _floorSelectionController,
                        out NetherStartStatusParentObservation parent
                    ...
                    || parent.State != NetherStartStatusParentState.Pending)
                {
                    ...
                    _recoveredCheckpointHandoffPrepared = false;
                    return NetherNativeActionResult.BindingUnavailable(
                        "recovered-checkpoint-adoption-evidence-changed"
                    );
                }

                _popupOwnership.BeginOwner(action.Kind, ownerGeneration);
                NetherPopupOwnership ownership = _popupOwnership.RegisterAfter(
                    existingContinue.Popup,
                    action.Kind,
                    ownerGeneration,
                    minimumSequence
                );
                if (ownership.Sequence <= minimumSequence)
                {
                    _checkpointFlow.Clear();
                    _recoveredCheckpointHandoffPrepared = false;
                    return NetherNativeActionResult.BindingUnavailable(
                        "recovered-checkpoint-adoption-sequence-unavailable"
                    );
                }
                _popupSequence = Math.Max(_popupSequence, ownership.Sequence);
                _continuePopup = existingContinue with
                {
                    Sequence = ownership.Sequence,
                    OwnerAction = action.Kind,
            ...
            _checkpointMinimumSequence = minimumSequence;
        ...
        if (adoptedRecoveredParent)
        {
            NetherAutoClimbController.LogDiagnostic(
                "checkpoint-native",
                new("stage", "adopt-recovered-parent"),
                new("action", action.Kind.ToString()),
                new("ownerGeneration", _checkpointOwnerGeneration.ToString()),
                new("minimumSequence", _checkpointMinimumSequence.ToString()),
                new("continueSequence", (_continuePopup?.Sequence ?? 0).ToString()),
                new("lockReward", action.ReturnLockReward.ToString())
            );
            return NetherNativeActionResult.Started(
                "adopted-recovered-start-status-checkpoint-parent"
            );
        }
    ...
    private NetherNativeActionResult PollCheckpointFlow()
    {
        ...
        NetherCheckpointNativeStage nextStage = _checkpointFlow.Stage;
        string signature = action.Kind + "|" + stage + "|" + nextStage + "|"
            + result.Kind + "|" + result.Detail;
        if (_checkpointDiagnosticGate.ShouldEmit("native", signature))
        {
            NetherAutoClimbController.LogDiagnostic(
                "checkpoint-native",
                new("stage", stage.ToString()),
                new("nextStage", nextStage.ToString()),
                new("action", action.Kind.ToString()),
                new("outcome", result.Kind.ToString()),
                new("detail", result.Detail),
                new("ownerGeneration", ownerGeneration.ToString()),
    ...
    private NetherNativeActionResult PollCheckpointParentTask()
    {
        if (_checkpointStartStatusParentSource == CheckpointStartStatusParentSource.Recovered)
        {
            ...
            {
                return NetherNativeActionResult.BindingUnavailable(
                    "recovered-checkpoint-parent-owner-lost"
                );
            }
        ...
        NetherCheckpointPopupObservation? observation = registration is PopupRegistration current
            ? new NetherCheckpointPopupObservation(
                ...
                current.IsLive
                    ...
                    && current.RuntimeGeneration == _runtimeGeneration
            )
            : null;
    ...
    {
        if (_pendingCheckpointAction?.Kind == NetherActionKind.Continue
            && _continueSceneTransition.TryObserveExpectedTerminalNativeParentAfterOwnerTransition(result))
        {
            ...
            // cancellation or exact source-less pooled-parent fault reaches the generated
            // HandleStartEventByStatusAsync parent.
            CompleteCheckpointNativeFlow();
            return NetherNativeActionResult.Completed(
                "checkpoint-native-flow-terminal-after-owner-transition:" + result.Detail
            );
        }
    ...
    /// <summary>
    /// Derives an optional post-Continue prediction from server-authoritative current floor data
    /// plus the packaged master link <c>m_nether_map_floor_id_next</c>.  The native Continue API
    /// does not submit either ID, so an absent/broken local link yields null and the destination
    /// is instead proven from the server response after the exact native parent completes.
    /// </summary>
    ...
    )
    {
        ...
        {
            // A missing local prediction is not an inferred destination.  The snapshot carries
            // null and post-Continue reconciliation requires a new positive server identity.
            return null;
        }
    }
    ...
    private static NetherRuntimePopupResult TryMapEventPopup(
        ...
        NetherRuntimePopupKind kind,
        int rawFloorType,
        NetherRuntimeNativeEventPopupCaptureFactory? nativeCaptureFactory,
        NetherStrategyTypedSemanticProviderEvidence? typedSemanticProvider,
        NetherSnapshotFingerprint? authoritativeSnapshotFingerprint
    )
    {
        if (nativeCaptureFactory == null)
            return TryMapEventPopup(
                registration,
                kind,
                rawFloorType,
                typedSemanticProvider
        ...
        NetherRuntimeNativeEventPopupCapture? capture;
        try
        {
            capture = nativeCaptureFactory(registration.Controller, kind);
        }
        catch (Exception ex)
        {
            return NetherRuntimePopupResult.Failure(
                "managed-event-popup-capture-exception:" + ex.GetType().Name
            );
        }
        if (capture == null)
            return NetherRuntimePopupResult.Failure("managed-event-popup-capture-unavailable");
        if (authoritativeSnapshotFingerprint is not NetherSnapshotFingerprint currentFingerprint
            || capture.SnapshotFingerprint is not NetherSnapshotFingerprint captureFingerprint
            || captureFingerprint != currentFingerprint)
        {
            return NetherRuntimePopupResult.Failure("stale-managed-event-popup-capture");
        }
        return TryMapEventPopup(capture, kind, rawFloorType, typedSemanticProvider);
    }

    /// <summary>
    /// Pure managed popup mapper used by an adapter that copied the native fields before entering
    /// the bridge. It deliberately keeps raw battle type/rarity as diagnostics only; the semantic
    /// tier lookup is the same duplicate/conflict-safe provider seam used by visible evidence.
    /// </summary>
    private static NetherRuntimePopupResult TryMapEventPopup(
        NetherRuntimeNativeEventPopupCapture capture,
        NetherRuntimePopupKind kind,
    ...
    )
    {
        if (kind == NetherRuntimePopupKind.Event
            && capture.TargetCharacterId <= 0)
        {
            return NetherRuntimePopupResult.Failure("missing-managed-event-target-character-id");
        }
        if (capture.EventId <= 0
            || capture.DeclaredPartIds == null
            || capture.Parts == null
            || capture.Battles == null
            || capture.Items == null)
        {
            return NetherRuntimePopupResult.Failure("invalid-managed-event-popup-capture");
        }
        if (!TryValidateManagedEventPartShape(
                capture.DeclaredPartIds,
                out string partShapeError
            ))
        {
            return NetherRuntimePopupResult.Failure(partShapeError);
        }

        Dictionary<long, NetherRuntimeNativeEventPart> partById = BuildRuntimeRows(
            capture.Parts,
            part => part.Id,
            out HashSet<long> ambiguousPartIds
        );
        Dictionary<long, NetherRuntimeNativeBattle> battleById = BuildRuntimeRows(
            capture.Battles,
            battle => battle.Id,
            out HashSet<long> ambiguousBattleIds
        );
        Dictionary<long, NetherRuntimeNativeItem> itemById = BuildRuntimeRows(
            capture.Items,
            item => item.Id,
        ...
        for (int index = 0; index < capture.DeclaredPartIds.Count; index++)
        {
            long partId = capture.DeclaredPartIds[index];
            if (partId == 0)
            {
                foundEmptyPart = true;
                continue;
            ...
            string? referenceError = partId < 0
                ? "invalid-managed-event-part-reference"
                : foundEmptyPart
                    ? "noncontiguous-managed-event-part-reference"
                    : !seenDeclared.Add(partId)
                        ? "duplicate-managed-event-part-reference:" + partId
                        : ambiguousPartIds.Contains(partId)
                            ? "duplicate-managed-event-part-row:" + partId
                            : null;
            if (referenceError != null || !partById.TryGetValue(partId, out NetherRuntimeNativeEventPart? part))
            {
                options.Add(RuntimeUnknownEventOption(
                    capture.EventId,
                    Math.Max(0, partId),
                    index + 1,
                    referenceError ?? "missing-managed-event-part-row:" + partId
                ));
                continue;
            }

            bool mapped = TryMapManagedEventPart(
                part,
                itemById,
                ambiguousItemIds,
                battleById,
        ...
        if (options.Count == 0)
            return NetherRuntimePopupResult.Failure("empty-managed-event-part-array");
    ...
    private static bool TryValidateManagedEventPartShape(
        IReadOnlyList<long> declaredPartIds,
        out string detail
    )
    {
        detail = string.Empty;
        if (declaredPartIds == null || declaredPartIds.Count != 4)
        {
            detail = "invalid-managed-event-part-shape:expected-four-slots";
            return false;
        }

        bool foundTrailingSentinel = false;
        var seen = new HashSet<long>();
        foreach (long partId in declaredPartIds)
        {
            if (partId < 0)
            {
                detail = "invalid-managed-event-part-shape:negative-part-id";
                return false;
            }
            if (partId == 0)
            {
                foundTrailingSentinel = true;
                continue;
            }
            if (foundTrailingSentinel)
            {
                detail = "invalid-managed-event-part-shape:noncontiguous-sentinel";
                return false;
            }
            if (!seen.Add(partId))
            {
                detail = "invalid-managed-event-part-shape:duplicate-part-id:" + partId;
                return false;
            }
        }
        if (seen.Count == 0)
        {
            detail = "invalid-managed-event-part-shape:missing-part-id";
            return false;
        }
        return true;
    }

    private static bool TryMapManagedEventPart(
        NetherRuntimeNativeEventPart part,
        IReadOnlyDictionary<long, NetherRuntimeNativeItem> itemById,
        IReadOnlySet<long> ambiguousItemIds,
        IReadOnlyDictionary<long, NetherRuntimeNativeBattle> battleById,
    ...
    )
    {
        ...
        if (part == null || part.Id <= 0)
        {
            ...
            detail = "invalid-managed-event-part";
            return false;
        }

        var mapped = new List<NetherEffect>();
        if (!TryMapManagedTargetEffect(
                part.TargetType1,
        ...
        if (part.ContentType != 0)
        {
            if (part.Amount < 0)
            {
                detail = "invalid-managed-event-content";
                return false;
            }
            NetherEffect? contentEffect = part.ContentType switch
            {
                30 or 31 when part.ContentId > 0
                    && !ambiguousItemIds.Contains(part.ContentId)
                    && itemById.TryGetValue(part.ContentId, out NetherRuntimeNativeItem? item)
                    && TryMapTypedEventReward(
                        item.Id,
                        semanticTiers,
                        out int typedItemType,
                        out NetherRewardRarity typedRarity
                    ) => new NetherEffect(NetherEffectKind.Item, part.Amount)
                {
                ...
                160 when NetherEventNativeMapping.IsCodeOfferContentId(part.ContentId) => new NetherEffect(NetherEffectKind.AbyssCodeOffer, part.Amount)
                {
                    ContentId = part.ContentId,
                },
                165 when NetherEventNativeMapping.IsValidResourceContentId(part.ContentId) => new NetherEffect(NetherEffectKind.NetherGoldGain, part.Amount)
                {
                    ContentId = part.ContentId,
                },
                166 when NetherEventNativeMapping.IsValidResourceContentId(part.ContentId) => new NetherEffect(NetherEffectKind.TreasureKeyGain, part.Amount)
                {
                    ContentId = part.ContentId,
                },
                _ => null,
            ...
            {
                detail = "unsupported-managed-event-content-type:" + part.ContentType;
                return false;
            }
        ...
        {
            detail = "invalid-managed-event-effect-count:" + mapped.Count;
            return false;
        }
        effects = mapped;
        return true;
    ...
    private static bool TryMapTypedEventReward(
        long itemId,
        NetherStrategySemanticTierLookup semanticTiers,
        out int itemType,
    ...
    )
    {
        ...
        if (!semanticTiers.TryGetCanonicalRewardEvidence(
                itemId,
            ...
            return false;
        return tier is NetherCanonicalRewardTier.GoldRankFive or NetherCanonicalRewardTier.RedRankFive;
    }

    private static bool TryMapManagedTargetEffect(
        int rawType,
        long parameter,
        IReadOnlyDictionary<long, NetherRuntimeNativeBattle> battleById,
        IReadOnlySet<long> ambiguousBattleIds,
    ...
    )
    {
        ...
        if (kind == NetherEffectKind.Battle)
        {
            if (ambiguousBattleIds.Contains(parameter)
                || !battleById.TryGetValue(parameter, out NetherRuntimeNativeBattle? battle)
                || battle.Id <= 0
                || battle.BattleStageId <= 0
                || battle.CodeDropRatio < 0)
            {
                detail = "managed-event-battle-row-unavailable:" + parameter;
                return false;
            }
            NetherEventBattleEvidence battleEvidence;
            if (semanticTiers.TryGetEventBattleTier(battle.Id, out NetherEventBattleTier tier))
            {
            ...
            NetherEffect? contentEffect = part.content_type switch
            {
                ...
                30 or 31 when part.content_id > 0
                    ...
                    && TryMapTypedEventReward(
                        ...
                        out int typedItemType,
                        out NetherRewardRarity rarity
                    ) => new NetherEffect(NetherEffectKind.Item, (int)part.amount)
                {
    ...
    private static NetherRuntimePopupResult TryMapShopPopup(
        PopupRegistration registration,
        NetherStrategyTypedSemanticProviderEvidence? typedSemanticProvider,
        NetherRuntimeManagedShopPopupCaptureFactory? managedCaptureFactory,
        NetherSnapshotFingerprint? authoritativeSnapshotFingerprint
    )
    {
        if (managedCaptureFactory != null)
            return TryMapManagedShopPopup(
                registration,
                typedSemanticProvider,
                managedCaptureFactory,
                authoritativeSnapshotFingerprint
            );

        return TryMapNativeShopPopup(registration, typedSemanticProvider);
    }

    private static NetherRuntimePopupResult TryMapNativeShopPopup(
        PopupRegistration registration,
        NetherStrategyTypedSemanticProviderEvidence? typedSemanticProvider
    )
    {
        ...
        if (itemRows != null)
        {
            foreach (MItems item in itemRows)
            {
                if (item == null || item.id <= 0)
                    continue;
                int rawItemType = item.type is >= int.MinValue and <= int.MaxValue
                    ? (int)item.type
                    : 0;
                NetherRewardRarity rawRarity = item.rarity is >= 0 and <= (int)NetherRewardRarity.UniqueWeapon
                    ? (NetherRewardRarity)item.rarity
        ...
        NetherShopContentMapResult mapped = NetherShopContentMapper.Map(
            rows,
            itemById,
            typedSemanticProvider
        );
    ...
    private static NetherRuntimePopupResult TryMapManagedShopPopup(
        PopupRegistration registration,
        NetherStrategyTypedSemanticProviderEvidence? typedSemanticProvider,
        NetherRuntimeManagedShopPopupCaptureFactory captureFactory,
        NetherSnapshotFingerprint? authoritativeSnapshotFingerprint
    )
    {
        NetherRuntimeManagedShopPopupCapture? capture;
        try
        {
            capture = captureFactory(registration.Controller);
        }
        catch (Exception ex)
        {
            return NetherRuntimePopupResult.Failure(
                "managed-shop-popup-capture-exception:" + ex.GetType().Name
            );
        }
        if (capture == null)
            return NetherRuntimePopupResult.Failure("managed-shop-popup-capture-unavailable");
        if (authoritativeSnapshotFingerprint is not NetherSnapshotFingerprint currentFingerprint
            || capture.SnapshotFingerprint is not NetherSnapshotFingerprint captureFingerprint
            || captureFingerprint != currentFingerprint)
        {
            return NetherRuntimePopupResult.Failure("stale-managed-shop-popup-capture");
        }
        if (capture.Contents == null || capture.Items == null)
            return NetherRuntimePopupResult.Failure("invalid-managed-shop-popup-capture");

        var itemById = new Dictionary<long, NetherShopItemMaster>();
        foreach (NetherShopItemMaster item in capture.Items)
        {
            if (item.ItemId <= 0 || !itemById.TryAdd(item.ItemId, item))
                return NetherRuntimePopupResult.Failure("duplicate-managed-shop-item-master:" + item.ItemId);
        }
        NetherShopContentMapResult mapped = NetherShopContentMapper.Map(
            capture.Contents,
            itemById,
        ...
        if (!mapped.IsSuccess)
            return NetherRuntimePopupResult.Failure("managed-shop-map:" + mapped.Detail);
        return NetherRuntimePopupResult.Success(new NetherRuntimePopupContext
        {
            Kind = NetherRuntimePopupKind.Shop,
            ShopContents = mapped.Contents,
    ...
    private static NetherCodeMasterAudit CreateCodeMasterAudit(
        ...
        Project.AbilityAssetDataStore? commonAbilityStore
    )
    {
        long abilityId = 0;
        ...
        string situationTypes = "unavailable";
        ...
        try
        {
            Project.IAbilityEffectData? ability = row.effect_type switch
            {
                (int)NetherCodeMasterEffectType.NetherAbility =>
                    abilityStore?.GetAbilityEffectAsset(row.effect_parameter_1),
            ...
            if (ability != null)
            {
                ...
                if (string.IsNullOrEmpty(situationTypes))
                    situationTypes = "none";
                abilityEffectType = row.effect_parameter_2 is > 0 and <= int.MaxValue
                    ? RuntimeTypeIdentifier(
                        ability.GetAbilityEffect(checked((int)row.effect_parameter_2), 0)
                    )
                    : "invalid-ability-level";
            }
        }
        catch (Exception ex)
        {
    ...
    private static string CreateCodeHash(IEnumerable<NetherCodeState> codes) =>
